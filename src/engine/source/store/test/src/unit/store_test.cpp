...
base::Name removePrefix(const base::Name& name, std::size_t level = 1)
{
    ...
    {
        throw std::runtime_error("Cannot remove prefix from name: " + name.toStr());
    }
...
{
protected:
    void SetUp() override { logger::testInit(); }
};
...
{
    EXPECT_CALL(*driver, readDoc(rDoc_1A)).WillOnce(testing::Return(driverReadDocResp(Doc(R"({"name": "doc_1A"})"))));
...
{
    EXPECT_CALL(*driver, readDoc(base::Name("x"))).WillOnce(testing::Return(driverReadDocResp(Doc(R"({"name": "doc_1A"})"))));
