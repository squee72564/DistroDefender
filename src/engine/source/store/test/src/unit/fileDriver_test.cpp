...
TEST_F(FileDriverTest, GetCollection)
{
    ...
    std::filesystem::create_directories(path);
    {
        ...
        file1 << TEST_JSON.toStr();
        std::ofstream file2(path2);
        file2 << TEST_JSON.toStr();
    }
    FileDriver fDriver(m_path);
    ...
    std::vector<std::string> expected {name2.toStr(), name1.toStr()};
    auto val1 = std::get<store::Col>(result)[0];
    auto val2 = std::get<store::Col>(result)[1];
    ASSERT_TRUE(std::find(expected.begin(), expected.end(), val1.toStr()) != expected.end());
    ASSERT_TRUE(std::find(expected.begin(), expected.end(), val2.toStr()) != expected.end());
}
