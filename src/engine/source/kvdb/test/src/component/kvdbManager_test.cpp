...
TEST_F(KVDBManagerTest, InitializeDBInUseWithOtherManager)
{
    ...
    auto kvdbManager = std::make_shared<kvdbManager::KVDBManager>(kvdbManagerOptions);
...
TEST_F(KVDBManagerTest, DeleteDB)
{
    ...
    ASSERT_EQ(std::get<base::Error>(result).message, "The DB 'DeleteDB' does not exist.");
}
...
TEST_F(KVDBManagerTest, DoubleDeleteDB)
{
    ...
    ASSERT_EQ(result->message, "The DB 'DoubleDeleteDB' does not exist.");
}
