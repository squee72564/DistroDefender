...
protected:
    ...
    auto getManagerWithDb(const std::string& path, Type type)
    {
        ...
        docJson.setType(TYPE_PATH, typeName(type));
...
TEST_F(GeoManagerTest, InitializeAddingDbs)
{
    ...
    json::Json doc1;
    doc1.setType(PATH_PATH, doc1File);
    doc1.setType(TYPE_PATH, "asn");
    doc1.setType(HASH_PATH, "hash1");
    ...
    json::Json doc2;
    doc2.setType(PATH_PATH, doc2File);
    doc2.setType(TYPE_PATH, "city");
    doc2.setType(HASH_PATH, "hash2");
...
TEST_F(GeoManagerTest, RemoteUpsertDb)
{
    ...
    auto dbType = Type::ASN;
    auto dbPath = std::filesystem::path(dbFile).string();
    std::string_view hash = "hash";
    std::string_view dbUrl = "dbUrl";
    std::string_view hashUrl = "hashUrl";
    auto content = getContentDb(dbFile);
    auto internalName = base::Name(INTERNAL_NAME) + base::Name(std::filesystem::path(dbFile).filename().string());

    EXPECT_CALL(*mockDownloader, downloadMD5(Eq(hashUrl))).WillOnce(testing::Return(base::RespOrError<std::string>(std::string{hash})));
    EXPECT_CALL(*mockDownloader, downloadHTTPS(Eq(dbUrl)))
        .WillOnce(testing::Return(base::RespOrError<std::string>(content)));
    EXPECT_CALL(*mockDownloader, computeMD5(Eq(content))).WillOnce(testing::Return(std::string{hash})).WillOnce(testing::Return(std::string{hash}));
    EXPECT_CALL(*mockStore, upsertInternalDoc(internalName, testing::_)).WillOnce(testing::Return(storeOk()));
...
TEST_F(GeoManagerTest, RemoteUpsertDbAlreadeyUpdated)
{
    ...
    auto dbPath = std::filesystem::path(dbFile).string();
    auto dbType = Type::ASN;
    std::string_view dbHash = "hash";
    std::string_view hashUrl = "hashUrl";
    auto dbDoc = json::Json{};
    dbDoc.setType(PATH_PATH, dbPath);
    dbDoc.setType(TYPE_PATH, typeName(dbType));
    dbDoc.setType(HASH_PATH, dbHash);
    auto internalName = base::Name(INTERNAL_NAME) + base::Name(std::filesystem::path(dbFile).filename().string());

    auto manager = getManagerWithDb(dbPath, dbType);

    EXPECT_CALL(*mockDownloader, downloadMD5(Eq(hashUrl)))
        .WillOnce(testing::Return(base::RespOrError<std::string>(std::string{dbHash})));
    EXPECT_CALL(*mockStore, readInternalDoc(internalName)).WillOnce(testing::Return(storeReadDocResp(dbDoc)));

    base::OptError error;
    ASSERT_NO_THROW(error = manager.remoteUpsertDb(dbPath, dbType, "dbUrl"sv, std::string{hashUrl}));
    ASSERT_FALSE(base::isError(error));
    ASSERT_EQ(manager.listDbs().size(), 1);
    ASSERT_EQ(manager.listDbs()[0].name, std::filesystem::path(dbFile).filename().string());
    ASSERT_EQ(manager.listDbs()[0].type, dbType);
...
TEST_F(GeoManagerTest, RemoteUpsertDbErrorTypeUsed)
{
    ...
    base::OptError error;
    ASSERT_NO_THROW(error = manager.remoteUpsertDb("any"sv, Type::ASN, "dbUrl"sv, "hashUrl"sv));
    ASSERT_TRUE(base::isError(error));
    ASSERT_EQ(manager.listDbs().size(), 1);
    ASSERT_EQ(manager.listDbs()[0].name, std::filesystem::path(dbFile).filename().string());
...
TEST_F(GeoManagerTest, RemoteUpsertDbFailOneHash)
{
    ...
    EXPECT_CALL(*mockDownloader, downloadHTTPS(Eq(dbUrl)))
        .WillOnce(testing::Return(base::RespOrError<std::string>(content)))
        .WillOnce(testing::Return(base::RespOrError<std::string>(content)));
    EXPECT_CALL(*mockDownloader, computeMD5(Eq(content)))
        .WillOnce(testing::Return("other_hash"))
        .WillOnce(testing::Return(std::string{hash}))
        .WillOnce(testing::Return(std::string{hash}));
    EXPECT_CALL(*mockStore, upsertInternalDoc(internalName, testing::_)).WillOnce(testing::Return(storeOk()));
...
TEST_F(GeoManagerTest, RemoteUpsertDbErrorWriting)
{
    ...
    auto dbFile = getTmpDb();
    auto dbType = Type::ASN;
    std::string_view dbPath = "non_existent_file";
    std::string_view hash = "hash";
    std::string_view dbUrl = "dbUrl";
    ...
    EXPECT_CALL(*mockDownloader, downloadHTTPS(Eq(dbUrl)))
        .WillOnce(testing::Return(base::RespOrError<std::string>(content)));
    EXPECT_CALL(*mockDownloader, computeMD5(Eq(content))).WillOnce(testing::Return(std::string{hash}));
