...
#include <geo/mockDownloader.hpp>

namespace
{

constexpr std::string_view g_maxmindDbPath {MMDB_PATH_TEST};

constexpr std::string_view g_ipFullData {"1.2.3.4"};
constexpr std::string_view g_ipFullData2 {"1.2.3.5"};
constexpr std::string_view g_ipNotFound {"1.2.3.6"};

bool compareLookupResult(const MMDB_lookup_result_s& res1, const MMDB_lookup_result_s& res2)
{
    return  (
        res1.found_entry == res2.found_entry &&
        res1.netmask == res2.netmask &&
        res1.entry.mmdb == res2.entry.mmdb
    );
}
...
protected:
    ...
    void SetUp() override
    {
        ...
        json::Json docJson;
        docJson.setType(PATH_PATH, path);
        docJson.setType(TYPE_PATH, typeName(Type::CITY));
        docJson.setType(HASH_PATH, "hash");
    ...
    void testAllGetBehavesEqual(std::string_view ip, bool success)
    {
        decltype(locator->getString({}, {})) resStr;
        ASSERT_NO_THROW(resStr = locator->getString(ip, "test_map.test_str1"sv));

        decltype(locator->getUint32({}, {})) resUint;
        ASSERT_NO_THROW(resUint = locator->getUint32(ip, "test_uint32"sv));

        decltype(locator->getDouble({}, {})) resDouble;
        ASSERT_NO_THROW(resDouble = locator->getDouble(ip, "test_double"sv));

        decltype(locator->getAsJson({}, {})) resJson;
        ASSERT_NO_THROW(resJson = locator->getAsJson(ip, "test_map.test_str1"sv));

        if (success)
        {
            ASSERT_FALSE(base::isError(resStr)) << base::getError(resStr).message;
            ASSERT_EQ("DistroDefender", base::getResponse<std::string>(resStr));
            ...
            ASSERT_EQ(json::Json(R"("DistroDefender")"), base::getResponse<json::Json>(resJson));
        }
        else
        {
...
TEST_F(LocatorTest, GetUpdatesCache)
{
    ...
    ASSERT_NO_THROW(locator->getString(g_ipFullData, "test_map.test_str1"sv));
    ...
    ASSERT_NO_THROW(locator->getString(g_ipFullData2, "test_map.test_str1"sv));
    ...
    ASSERT_NO_THROW(locator->getString(g_ipNotFound, "test_map.test_str2"sv));
    res = locator->getCachedResult();
    ASSERT_FALSE(compareLookupResult(prev, res));
...
TEST_F(LocatorTest, GetString)
{
    ...
    ASSERT_NO_THROW(res = locator->getString(g_ipFullData, "not_found"sv));
    ASSERT_TRUE(base::isError(res));

    ASSERT_NO_THROW(res = locator->getString(g_ipFullData, "test_uint32"sv));
    ASSERT_TRUE(base::isError(res));
}
...
TEST_F(LocatorTest, GetUint32)
{
    ...
    ASSERT_NO_THROW(res = locator->getUint32(g_ipFullData, "not_found"sv));
    ASSERT_TRUE(base::isError(res));

    ASSERT_NO_THROW(res = locator->getUint32(g_ipFullData, "test_map.test_str1"sv));
    ASSERT_TRUE(base::isError(res));
}
...
TEST_F(LocatorTest, GetDouble)
{
    ...
    ASSERT_NO_THROW(res = locator->getDouble(g_ipFullData, "not_found"sv));
    ASSERT_TRUE(base::isError(res));

    ASSERT_NO_THROW(res = locator->getDouble(g_ipFullData, "test_map.test_str1"sv));
    ASSERT_TRUE(base::isError(res));

    ASSERT_NO_THROW(res = locator->getDouble(g_ipFullData, "test_uint32"sv));
    ASSERT_TRUE(base::isError(res));
}
...
TEST_F(LocatorTest, GetAsJson)
{
    ...
    ASSERT_NO_THROW(res = locator->getAsJson(g_ipFullData, "not_found"sv));
    ASSERT_TRUE(base::isError(res));

    ASSERT_NO_THROW(res = locator->getAsJson(g_ipFullData, "test_map.test_str1"sv));
    ASSERT_FALSE(base::isError(res));
    expected.setType("", "DistroDefender");
    ASSERT_EQ(expected, base::getResponse<json::Json>(res));

    ASSERT_NO_THROW(res = locator->getAsJson(g_ipFullData, "test_map"sv)); // Complex type
    ASSERT_TRUE(base::isError(res));

    ASSERT_NO_THROW(res = locator->getAsJson(g_ipFullData, "test_array"sv)); // Complex type
    ASSERT_TRUE(base::isError(res));

    ASSERT_NO_THROW(res = locator->getAsJson(g_ipFullData, "test_uint32"sv));
    ASSERT_FALSE(base::isError(res));
    expected.setType("", 94043);
    ASSERT_EQ(expected, base::getResponse<json::Json>(res));

    ASSERT_NO_THROW(res = locator->getAsJson(g_ipFullData, "test_double"sv));
    ASSERT_FALSE(base::isError(res));
    expected.setType("", 37.386);
    ASSERT_EQ(expected, base::getResponse<json::Json>(res));

    ASSERT_NO_THROW(res = locator->getAsJson(g_ipFullData, "test_float"sv));
    ASSERT_FALSE(base::isError(res));
    expected.setType("", static_cast<float>(122.0838f));
    ASSERT_EQ(expected.toStr(), base::getResponse<json::Json>(res).toStr());
    //ASSERT_EQ(expected, base::getResponse<json::Json>(res));

    ASSERT_NO_THROW(res = locator->getAsJson(g_ipFullData, "test_bytes"sv));
    ASSERT_FALSE(base::isError(res));
    expected.setType("", "abcd");
    ASSERT_EQ(expected, base::getResponse<json::Json>(res));

    ASSERT_NO_THROW(res = locator->getAsJson(g_ipFullData, "test_uint16"sv));
    ASSERT_FALSE(base::isError(res));
    expected.setType("", 123);
    ASSERT_EQ(expected, base::getResponse<json::Json>(res));

    ASSERT_NO_THROW(res = locator->getAsJson(g_ipFullData, "test_uint64"sv));
    ASSERT_FALSE(base::isError(res));
    expected.setType("", "1234567890");
    ASSERT_EQ(expected, base::getResponse<json::Json>(res));

    ASSERT_NO_THROW(res = locator->getAsJson(g_ipFullData, "test_uint128"sv));
    ASSERT_FALSE(base::isError(res));
    expected.setType("", "0x0000000000000000ab54a98ceb1f0ad2");
    ASSERT_EQ(expected.toStr(), base::getResponse<json::Json>(res).toStr());

    ASSERT_NO_THROW(res = locator->getAsJson(g_ipFullData, "test_boolean"sv));
    ASSERT_FALSE(base::isError(res));
    expected.setType("", true);
    ASSERT_EQ(expected, base::getResponse<json::Json>(res));
}
