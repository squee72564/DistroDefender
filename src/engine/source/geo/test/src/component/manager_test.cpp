...
protected:
    ...
    void SetUp() override
    {
        ...
        json::Json asnJson;
        asnJson.setType(PATH_PATH, asnPath);
        asnJson.setType(TYPE_PATH, typeName(Type::ASN));
        asnJson.setType(HASH_PATH, "asnHash");

        json::Json cityJson;
        cityJson.setType(PATH_PATH, cityPath);
        cityJson.setType(TYPE_PATH, typeName(Type::CITY));
        cityJson.setType(HASH_PATH, "cityHash");
...
TEST_F(GeoManagerTest, MultithreadDeleteAddLookup)
{
    ...
    auto locatorFn = [&errorMsg, &error, times](std::shared_ptr<ILocator> locator)
    {
        for (auto i = 0; i < times; i++)
        {
            ...
            auto res = locator->getString(g_ipFullData, DotPath{"test_map.test_str1"});
            if (base::isError(res))
            {
            ...
            {
                if (base::getResponse(res) != "DistroDefender")
                {
                    setError(
                        error,
                        errorMsg,
                        fmt::format("Locator thread got response '{}' which is not 'DistroDefender'", base::getResponse(res)));
                }
            }
        }
    };
...
TEST_F(GeoManagerTest, MultithreadListLookup)
{
    ...
    threads.emplace_back(
        [&error, &errorMsg, times, dbPath](std::shared_ptr<IManager> manager)
        {
            for (int i = 0; i < times; i++)
            {
                if (error.load())
            ...
            else
            {
                ...
                else
                {
                    if (base::getResponse(res) != "DistroDefender")
                    {
                        setError(error,
                                 errorMsg,
                                 fmt::format("Locator thread got response '{}' which is not 'DistroDefender'",
